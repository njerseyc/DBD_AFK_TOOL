...
def begin():
    save_cfg()
    afk = Process(target=AFK, args=(dbd_window, settings, custom_select, character_list, character_num_b), daemon=True)
    afk.start()
    AFK_pid.valve = afk.pid
    afk.join()
    hotkey.join()
...
def exit_program():
    psutil.Process(hotkey_pid.valve).terminate()
    psutil.Process(AFK_pid.valve).terminate()
...
def initialize():
    ...
    if not os.path.exists(CFG_PATH):
        ...
        settings.setValue("CPCI/rb_noaction_mode", False)
...
def save_cfg():
    ...
    settings.setValue("CPCI/rb_noaction_mode", dbd_window.main_ui.rb_noaction_mode.isChecked())
...
def read_cfg():
    ...
    dbd_window.main_ui.rb_noaction_mode.setChecked(json.loads(settings.value("CPCI/rb_noaction_mode")))
    ...
    if settings.value("CPCI/rb_survivor") == "true":
        ...
        dbd_window.main_ui.rb_noaction_mode.setDisabled(True)
...
def listen_key():
    ...
    #     listener.join()
    def on_begin():
        status_code.valve = True
    def on_pause():
        pause = psutil.Process(AFK_pid.valve)
        pause.suspend()
    def on_resume():
        resume = psutil.Process(AFK_pid.valve)
        resume.resume()
    def on_stop():
        stop_afk = psutil.Process(AFK_pid.valve)
        stop_afk.terminate()

    add_hotkey('alt+t', on_begin)
    add_hotkey('alt+p', on_pause)
    add_hotkey('alt+r', on_resume)
    add_hotkey('alt+s', on_stop)
    while True:
        pass
...
def killer_action():
    ...
    if custom_select.select_killer_lst[character_num_b.value - 1] in need_lst:
        act_move = random_movement()
        key_down(hwnd, act_move)
        act_direction = random_direction()
        key_down(hwnd, act_direction)
        ...
        key_up(hwnd, act_direction)
        key_up(hwnd, act_move)
    elif eq(custom_select.select_killer_lst[character_num_b.value - 1], "枯萎者"):
        for i in range(3):
            py.mouseDown(button='right')
            py.mouseUp(button='right')
            time.sleep(0.7)
        ...
        py.mouseDown()
        py.mouseUp()
    elif eq(custom_select.select_killer_lst[character_num_b.value - 1], "恶骑士"):
        py.mouseDown(button='right')
        py.mouseUp(button='right')
        time.sleep(0.3)
        for i in range(2):
...
def character_selection(character_list, character_num_b):
    '''自选特定的角色轮转【屠夫推荐】'''
    ghX = [405, 548, 703, 854]
    ghY = [314, 323, 318, 302]
    ...
    if eq(judge, 0):
        ...
        character_list = custom_select.match_select_killer_lst
        judge = 1
    py.sleep(1)
    moveclick(1, 1, 1, 1)
    moveclick(141, 109, 1, 1)  # 角色按钮
    timey = floordiv(character_list[character_num_b.value], 4)  # 取整
    timex = mod(character_list[character_num_b.value], 4)  # 取余
    time.sleep(0.5)
    ...
    if lt(timey, 6):  ## 最大的换行次数+1
        ...
        if eq(timey, 6) and eq(timex, 0):
            ...
            final_number = character_list[character_num_b] - 24
            # 多了两个坐标，减去。【多几个减几个】
            if gt(final_number, 1):
                final_number -= 2  # -->>
                moveclick(glX[final_number], glY[final_number], 1.5)
    if lt(circle, len(character_list)):
        circle += 1
        character_num_b += 1
        if eq(character_num_b, len(character_list)):
            circle = 0
            character_num_b = 0


def AFK(character_list, character_num_b):
    ...
    select_number = len(custom_select.select_killer_lst)
    if not custom_select.select_killer_lst and eq(settings.value("CPCI/rb_killer"), True):
        MessageBox(0, "最少选择一个屠夫", "提醒", win32con.MB_ICONASTERISK)
        sys.exit(0)
    # 置顶
    win32gui.SetWindowPos(hwnd, win32con.HWND_TOPMOST, 0, 0, 0, 0, win32con.SWP_NOSIZE | win32con.SWP_NOMOVE)
    # 取消置顶
        ...
        while matching == False:
            ...
            if eq(blood_and_ceasma(), True):
                ...
                if eq(settings.value("CPCI/rb_killer"), True):
                    time.sleep(1)
                    if gt(select_number, 1):
                        character_selection(character_list, character_num_b)
                    elif eq(select_number, 1):
                        character_selection(character_list, character_num_b)
                        select_number -= 1
...
if __name__ == '__main__':
    ...
    qss_style = '''
        ...
        '''
    hwnd = win32gui.FindWindow(None, u"DeadByDaylight  ")
    character_list = manager.list([])
    character_num_b = manager.Value('i', 0)  # 列表的下标
    AFK_pid = Value('i', 0)
    hotkey_pid = Value('i', 0)
    status_code = Value('c', False)
    hotkey = Process(target=listen_key)  #  args=(os.getpid(),)
    # authorization()
